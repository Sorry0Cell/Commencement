...
from utils import complete_stk_id, read_txt

full_col = ["Stkcd", "Trddt", "Opnprc", "Hiprc", "Loprc", "Clsprc", "Volume", "Turnover", "PB", "PE", "ROA", "ROE"]
feat_col = ["Opnprc", "Hiprc", "Loprc", "Clsprc", "Volume", "Turnover", "PB", "PE", "ROA", "ROE"]
...
def get_df(f_path, col_dt="Trddt", need_stk_id_file="../data/stk_id_201417_zhongzhengyibai.txt",
           col_stk_id="Stkcd", start_date=None, end_date=None):
    """
    :param f_path:
    :param col_dt:
    :param need_stk_id_file:
    :param col_stk_id:
    :param start_date:
    :param end_date:
    :return:
    ...
    # 再按股票代码取
    if need_stk_id_file.endswith(".csv"):
        # 只要300家
        need_stk_id_df = pd.read_csv(need_stk_id_file, header=None)
        need_stk_ids = need_stk_id_df[0].tolist()
    else:
        # 183家中证100
        need_stk_ids = read_txt(need_stk_id_file)
    need_stk_ids = [complete_stk_id(x) for x in need_stk_ids]

    df[col_stk_id] = df[col_stk_id].apply(complete_stk_id)
    res_df = []
    for i, x in enumerate(need_stk_ids):
        tmp_df = df.loc[df[col_stk_id] == x]
        if tmp_df.shape[0] > 0:
            logging.info("{} : {} df shape {} . ".format(i+1, x, tmp_df.shape))
            res_df.append(tmp_df)

    res_df = res_df[0].append(res_df[1:]) if len(res_df) > 1 else res_df[0]
    logging.info("res_df shape: {} . ".format(res_df.shape))
    return res_df
...
def gen_model_data(df, input_window=7, label_window=1,
    ...
    for stk_id in stk_ids:
        ...
        for i in range(input_window, tmp_len-label_window+1):
            ...
            price_diff = tmp_clsp[i+label_window-1] - tmp_clsp[i-1]     # label: 根据收盘价是涨是跌来计算
...
if __name__ == '__main__':
    ...
    parser.add_argument("--csv_input", default="../data/All_data_201418_1219.csv",
    ...
    parser.add_argument("--data_date_end", default="2017/12/31",
    ...
    parser.add_argument("--train_date_end", default="2016/12/31",
    ...
    main(args, save_norm_res=False, save_split_res=False,
         train_file_name="201417_train_{}_{}_1219.pkl".format(args.input_window, args.label_window),
         test_file_name="201417_test_{}_{}_1219.pkl".format(args.input_window, args.label_window))
